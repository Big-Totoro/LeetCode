...
public class _505 {
    ...
    public static class Solution3 {
        ...
        public int shortestDistance(int[][] maze, int[] start, int[] destination) {
            ...
            dijkstra(maze, start, distanceMaze);

            return (distanceMaze[destination[0]][destination[1]] == Integer.MAX_VALUE) ? -1 : distanceMaze[destination[0]][destination[1]];
        }

        private void dijkstra(int[][] maze, int[] start, int[][] distanceMaze) {
            int[][] directions = new int[][] {{0, 1}, {1, 0}, {0, -1}, {-1, 0}};
            ...
            while (!queue.isEmpty()) {
                ...
                int startRow = coords[0];
                int startCol = coords[1];
                int distance = coords[2];

                if (distance > distanceMaze[startRow][startCol]) {
                    continue;
                }
                ...
                for (int[] direction : directions) {
                    ...
                    int cellRow = row - direction[0];
                    int cellCol = col - direction[1];
                    int newDistance = distanceMaze[startRow][startCol] + steps;
                    if (isValid(maze, cellRow, cellCol) && distanceMaze[cellRow][cellCol] > newDistance) {
                        distanceMaze[cellRow][cellCol] = newDistance;
                        queue.offer(new int[] {cellRow, cellCol, newDistance});
                    }
                }
            }
        }
